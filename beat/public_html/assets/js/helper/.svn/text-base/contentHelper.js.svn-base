		...
		function paging_ajax_planmoderation(fungsi,start){
			$('.theContent').html('');
			$('.theContent').append('<div class="loaders"><img src="'+basedomain+'assets/images/loader.gif"></div>');
			$.post(basedomain+pages+"/ajax",{start:start,needs:"plan_moderation",publishedtype:publishedtype,startdate:startdate,enddate:enddate,search:search},function(data){
				if(data){
					if(data.result){
						var html = "";
						html += postmoderationviews(data.result);
