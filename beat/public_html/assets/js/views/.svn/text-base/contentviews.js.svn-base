...
function challengeHastagviews(data){
	...
	$.each(data,function(i,e){
		html+=" <tr>";
		html+="			<td>"+e.posted_date+"</td>";
		html+="			<td>"+e.author.name+" "+e.author.last_name+"</td>";
		html+="			<td>"+e.author.pagesdetail.brandid+"</td>";
		html+="			<td>"+e.author.pagesdetail.name+"</td>";
		html+="			<td><img src='"+e.image_full_path+"' width='200px' /></td>";
		html+="			<td>"+e.title+"</td>";
		html+="			<td class='alCenter'><input type='checkbox' name='test' id='test' ";
		html+="			userid='"+e.author.id+"' ";
		html+="			cid_user='"+e.id+"' ";
		html+="			cid='"+e.id+"' ";
		html+="			set='"+e.set+"' "; 
		html+="			/></td>";
...
function entourageviews(data){
	...
	$.each(data,function(i,e){
		...
		if(e.latestengagament.date)
			{
				var posted_date = e.latestengagament.date;
				var datesarr = posted_date.split(' ');
				var dates = datesarr[0].split('-');
				var times = datesarr[1].split(':');
...
function galleryviews(data){
	...
	$.each(data,function(i,e){
		...
		html +="                	<span class='desc1'>"+e.title+"</span>";
