	...
	function main(){
		...
		if($this->isUserOnline()){
			if($detect->isAndroidOS()){
				sendRedirect($CONFIG['ANDROID_APP']);
				die();
			}
			$page= strip_tags($this->_g('page'));	
			//check if pass ever change or not?
			$checkOldPass = $this->userHelper->checkuserpassword();
			//pr($checkOldPass);
			if(($page!='register'&&$checkOldPass==false)&&$page!='login'&&$page!='landing'&&$page!='changepassword'){
