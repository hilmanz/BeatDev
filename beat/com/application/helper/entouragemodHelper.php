...
class entouragemodHelper {
  ...
  function getBrand(){
	...
	function getEntourage($streid=null,$start=0,$limit=10,$all=false,$summary=false,$alldata=false){
		...
		{
			$startdate = strip_tags(date('Y-m-d',strtotime($this->apps->_p('startdate'))));
		}
		if($this->apps->_p('enddate'))
		{
			$enddate = strip_tags(date('Y-m-d',strtotime($this->apps->_p('enddate'))));
		}
		...
		if($this->apps->user->leaderdetail->type!=1) {
							...
							if($uid==0)	{
									...
									if($brandid>0)	$filterBrand = "AND ( mp.brandid IN ({$brandid}) OR mp.brandsubid IN ({$brandid}) ) ";
									if($plid>0)	$filterPL = "AND mp.otherid = {$plid}";
									
									if($this->apps->user->leaderdetail->type==2)	{
										 
										$filterPL = "AND mp.otherid = {$this->apps->user->id}";
									}
									
									if($this->apps->user->leaderdetail->type==3) {
										$branddetails = false;
										$branddetail = @$this->apps->user->branddetail;
										if($branddetail){
											foreach($branddetail as $val){
													$branddetails[$val->ownerid] = $val->ownerid;
											}
										}
										$brandids = false;
										if(is_array($branddetails)) 	{										 
											$brandids = implode(',',$branddetails);
										}
										if($brandids)$filterBrand = "AND  mp.brandid IN ({$brandids})  ";
									}
									
									if($this->apps->user->leaderdetail->type==5) 	{
										 
											$filterArea = " AND mp.city IN ({$this->apps->user->leaderdetail->city}) ";
		...
		WHERE 1 {$qUserid} {$qEntourage} {$qFilter} {$qFilterAlphabet} {$qDate}  ORDER BY entou.name ASC  {$qLimit} ";		
	...
	function addEntourage($img=false,$signature=false,$signatureba=false){
		...
		if($email!='') $updatedatavalidation[] = "email='{$email}'";
		...
		$sql =" SELECT referrerbybrand FROM my_entourage WHERE email=\"{$email}\" LIMIT 1";
		// //$this->logger->log($sql);
		$checkexisiting = $this->apps->fetch($sql);
		$isexisiting = 0;
		if($checkexisiting){
			$isexisiting = intval($checkexisiting['referrerbybrand']);
		}
		...
		if($entorourage){
			...
			$usertype = intval($entorourage['usertype']);
		}else return false;
		...
		$data['savemop'] = false;
			...
			if($mop['result']==1) {
				...
				$qreferrerbybrand = "";
				if(($isexisiting==0)&&($usertype==2)){
					 
						$qreferrerbybrand = ",referrerbybrand={$isexisiting}";
				}
				$verified = 1;
				$sql = "UPDATE my_entourage SET registerid='{$mop['data'][0]['RegistrationID']}',n_status=1{$qreferrerbybrand},usertype={$usertype},verified={$verified} WHERE id={$entourageid} LIMIT 1 ";
				
				// $this->logger->log("e stat :");
				// $this->logger->log($usertype);
				// $this->logger->log($isexisiting);
				//$this->logger->log($sql);
			...
			if($this->apps->user->id!=$isexisiting){
				if($usertype==2||$usertype==0){
					$version = 0;
					$sql = "SELECT MAX(version)mversion FROM my_circle LIMIT 1";
					$versions = $this->apps->fetch($sql);
						if($versions) if( $versions['mversion'] ) $version  = $versions['mversion']+1;
					
					$sql = "
					INSERT INTO my_circle (friendid,userid,ftype,groupid,date_time,n_status,version)
					VALUES ('{$entourageid}','{$this->apps->user->id}',0,0,NOW(),1,{$version})
					ON DUPLICATE KEY UPDATE n_status=1,version=VALUES(version)
					";
	...
	function synchenturage(){
		...
		// pr($val);exit;
			...
			$isexisiting = $val['referrerbybrand'];			
			...
			if($mop['result']==1) {
				...
				$qreferrerbybrand = "";
				if(($val['verified']==0)&&($usertype==2)){
						$isexisiting = 0;
						$qreferrerbybrand = ",referrerbybrand={$isexisiting}";
				}
				$verified = 1;
				$sql = "UPDATE my_entourage SET registerid='{$mop['data'][0]['RegistrationID']}',n_status=1{$qreferrerbybrand},usertype={$usertype},verified={$verified} WHERE id={$val['id']} LIMIT 1 ";
				...
				if($mop['result']==99) {
					...
					if($mop['data']){
						$verified =1;
						$sql = "UPDATE my_entourage SET n_status=2,verified={$verified}  WHERE id={$val['id']} LIMIT 1 ";
						$qData = $this->apps->query($sql);
			...
			if($this->apps->user->id!=$isexisiting){
				if($usertype==2){
					...
					$sql = "
					INSERT INTO my_circle (friendid,userid,ftype,groupid,date_time,n_status,version)
					VALUES ('{$val['id']}','{$val['referrerbybrand']}',0,0,NOW(),1,{$version})
					ON DUPLICATE KEY UPDATE n_status=1,version=VALUES(version)
					";
	...
	function synchenturage_batch(){
		...
		$sql = "SELECT * FROM my_entourage WHERE n_status = 0 ORDER BY register_date ASC LIMIT 50 ";
		$qData = $this->apps->fetch($sql,1);
			...
			if($mop['result']!=99) {
				foreach($mop['data'] as $keys=> $mopdata){
					...
					$Brand1U_ID = $qData[$keys]['Brand1U_ID'];
						// pr(" response code : ".$mopdata['ResponseCode']); 
					if($mopdata['ResponseCode']==1||$mopdata['ResponseCode']==11){
						if(@$mop['data'][$keys]['ResponseDescription']=='AutoUpdateProfileSuccess')$usertype=2;
						else $usertype=0;
						
						$qreferrerbybrand = "";
						if(($verified==0)&&($usertype==2)){
								$isexisiting = 0;
								$qreferrerbybrand = ",referrerbybrand={$isexisiting}";
						}
						if($verified==1){
						...
						if(($oldstatverified==0)&&($usertype==2)){
							$version = 0;
							$sql = "SELECT MAX(version)mversion FROM my_circle LIMIT 1";
							$versions = $this->apps->fetch($sql);
							if($versions) if( $versions['mversion'] ) $version  = $versions['mversion']+1;
							...
							$sql = "
							INSERT INTO my_circle (friendid,userid,ftype,groupid,date_time,n_status,version)
							VALUES ('{$entourageid}','{$referrerid}',0,0,NOW(),1,{$version})
							ON DUPLICATE KEY UPDATE n_status=1,version=VALUES(version)
							";
