...
class leaderboardHelper {
	...
	function getEntourageList(){
		...
		$sql = "SELECT count(*) totalentourage, sm.id,sm.name, sm.last_name, sm.img
				...
				WHERE pages.type = 1  AND entourage.n_status = 1 AND sm.n_status = 1
	...
	function topplacelist(){
		...
		if($this->apps->user->leaderdetail->type==1){
			$sql =" SELECT city FROM my_pages WHERE ownerid={$this->uid} LIMIT 1 ";
			
			$rqData = $this->apps->fetch($sql);
			// pr($rqData);
				if($rqData){
					$mycity = $rqData['city'];
					$sql =" SELECT provinceName FROM beat_city_reference WHERE id={$mycity} LIMIT 1";
					
					$qData = $this->apps->fetch($sql,1);
					$alluser = false;
					foreach($qData as $val){
						$alluser[$val['provinceName']] = "'{$val['provinceName']}'";
					}
					if($alluser) {
						$fuserid = implode(',',$alluser);
						$qUserid = " AND master.provinceName IN ({$fuserid}) ";
					}
				}
		} 
