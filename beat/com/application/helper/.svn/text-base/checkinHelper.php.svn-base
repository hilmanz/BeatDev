...
class checkinHelper {
	...
	function searchvenue($coor=null,$keywords=null){
		...
		WHERE 1  {$qKeywords}  AND n_status = 1
		GROUP BY vm.id 
	...
	function checkin(){
		...
		$version = 0;
		$sql = "SELECT MAX(version)mversion FROM {$this->dbshema}_news_content LIMIT 1";
		...
		if(!$contentid){
			...
			$sql ="
				INSERT INTO {$this->dbshema}_news_content (cityid,brief,title,content,tags,image,articleType,created_date,posted_date,authorid,fromwho,n_status,url,parentid,categoryid,version) 
				VALUES ('{$venueid}','{$brief}','{$title}',\"{$content}\",'','{$image}',{$type},NOW(),'{$posted_date}','{$authorid}','{$fromwho}',1,'',{$parentid},{$categoryid},{$version})
				";
			...
			if($contentid!=0){
				...
				if($checkd){
					...
					$sql = "UPDATE {$this->dbshema}_news_content SET image='{$image}',posted_date=NOW(),content='{$contentdata}',brief='{$brief}',version={$version} WHERE id = {$contentid} LIMIT 1";
					//$this->logger->log($sql);
					$this->apps->query($sql);
