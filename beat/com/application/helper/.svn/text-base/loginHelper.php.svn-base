	...
	*/
	...
	function getPagesStat($user_id=null,$leader_id=null){
		...
		if($data) {
			...
			$branddetail = false;
			if($topclass){
				$branddetail = $this->gethirarkidata(4,false,true);	
				...
				if($pagedata['leaderdetail']['brandid']!=0) $branddetail = $this->gethirarkidata($pagedata['leaderdetail']['brandid'],false,true);			
...
// pr($strpl);exit;					
			...
			// pr($pagedata);exit;			
			$brandsubdetail =false;
			if($topclass){
					$brandsubdetail = $this->gethirarkidata(5,false,true);		
			}else{
				...
				// else $brandsubdetail = $this->gethirarkidata($pagedata['leaderdetail']['brandsubid'],false,true);	
				 if($pagedata['leaderdetail']['brandsubid']!=0)$brandsubdetail = $this->gethirarkidata($pagedata['leaderdetail']['brandsubid'],false,true);	
			}
			
			if($brandsubdetail) $pagedata['branddetail'] = @array_merge($pagedata['branddetail'],$brandsubdetail);
			if($pagedata['branddetail']){
				$arrBranddetail = $pagedata['branddetail'];
				$pagedata['branddetail'] = array();
				foreach($arrBranddetail as $val){
						$pagedata['branddetail'][$val['id']] = $val;
				}
			}
			/*ba*/
			
			$arrBadetail = $pagedata['badetail'];
			$pagedata['badetail'] = array();
			foreach($arrBadetail as $val){
						$pagedata['badetail'][] = $val;
			}
			/*PL*/
			$arrPLdetail = $pagedata['pldetail'];
			$pagedata['pldetail'] = array();
			foreach($arrPLdetail as $val){
						$pagedata['pldetail'][] = $val;
			}
			/*areadetail*/
			$arrAreadetail = $pagedata['areadetail'];
			$pagedata['areadetail'] = array();
			foreach($arrAreadetail as $val){
						$pagedata['areadetail'][] = $val;
			}
			/*brand*/
			$arrBranddetail = $pagedata['branddetail'];
			$pagedata['branddetail'] = array();
