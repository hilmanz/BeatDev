...
class contentmodHelper {
	...
	function getAuthorProfile($otherid=null,$typeAuthor='admin'){
		...
		SELECT 
		pages.name, pages.id ,
		pages.type ,pages.img,pagetype.name role,
		...
		foreach($data as $key => $val){
			...
			if(!$data[$key]['img']) $data[$key]['img'] = "";
			if(!$data[$key]['brandid']) $data[$key]['brandid'] = "all";
			if(!$data[$key]['areaid']) $data[$key]['areaid'] = "all";
					...
					if($qDataUser){
						...
						if($data){
							foreach($data as $key => $val){
								...
								else $favoriteData[$key]['mylikes']=array();
	...
	function setWinnerChallenge(){
		...
		if($cid_user && $userid){
			...
			$point = $this->PointChallenge(null,$cid,"getpoint");
			 if($point) {
				if($point['prize']==0) $prize = $point['badgeid'];
				else $prize = $point['prize'];
			}
			//bot system halt
			...
			$sql ="
				...
				VALUES ({$userid},{$cid_user},'{$uid}',NOW(),'{$prize}',1)
			";
			// pr($sql);exit;
			$this->apps->query($sql);
			//$this->logger->log($sql);
		...
		if($cid){			
			...
			if($pathlike) {
				$sql ="	SELECT {$qFieldRecord} 
						FROM {$this->dbshema}_news_content_comment 
						WHERE contentid IN ({$cid}) AND n_status IN (0,1,2,3)
						{$qGroupRecord}
	...
	function getChallengeHashtag($start=0,$limit=5,$tags=null,$type=null){
		...
		if($tags){
			...
			$sql = "
				SELECT * FROM {$this->dbshema}_news_content anc	WHERE articleType = {$type} AND (title REGEXP '{$tags}' OR brief REGEXP '{$tags}' OR content REGEXP '{$tags}') AND n_status=1 ORDER BY posted_date DESC , id DESC 
				LIMIT {$start},{$limit}
			";
			// pr($sql);exit;
	...
	function getArticleContent($start=null,$limit=10,$contenttype=0,$topcontent=array(0,3),$articletype=false,$groupby=false,$author=false,$allcontent=false,$datetimes=false,$usingbadetail=true,$usingbranddetail=true,$checkin=true,$readarticle=false) {
		...
		$filter = $this->apps->searchmodHelper->filterEngine($limit,$groupby,$author);
		...
		if ($this->apps->_p('startdate')) {
			$start_date = strip_tags(date('Y-m-d',strtotime($this->apps->_p('startdate'))));
			
			$startdate = " AND DATE(anc.posted_date) >= DATE('{$start_date}') ";
		}
		if ($this->apps->_p('enddate')) {
			$end_date = strip_tags(date('Y-m-d',strtotime($this->apps->_p('enddate'))));
		...
		if ($this->apps->_request('publishedtype') || $this->apps->_request('publishedtype')!='' ) {
			$publishedtype = intval($this->apps->_request('publishedtype'));
			$qStatus = " AND anc.n_status =  {$publishedtype}  ";
			$qPublishStatus = " AND anc.n_status =  {$publishedtype}  ";
			if($this->apps->_request('publishedtype')=='3')
			{
			
				$qStatus = " AND anc.n_status  IN (3,0)  ";
				$qPublishStatus = " AND anc.n_status IN (3,0)  ";
			}
		}else{
			// frontend
		...
		if($articletype=='plan'){
			...
			$qStatus = " AND (anc.authorid = {$uid} OR anc.n_status IN (0,1))";	
			if($this->apps->_request('publishedtype')=='3')
			{
			...
			else if($this->apps->_request('publishedtype')=='1')
			{
				$qStatus = " AND anc.n_status = 1 ";
			}
			if($leadertype==4) {
				if($datetimes)  $qStatus = "  AND anc.n_status IN (0,1) ";		
			}
		...
		if($typeid=='4'){
				...
				if($this->apps->user->leaderdetail->type == 666 || $this->apps->user->leaderdetail->type == 4 ){
					$brandid = "4,5";
				}
		...
		if($typeid=='6') {
			if($this->apps->user->leaderdetail->type>=100) $qHeadid ="";
			else $qHeadid = "  AND anc.headid = {$this->apps->user->leaderdetail->brandid} ";
		}
		...
		FROM {$this->dbshema}_news_content anc 
		LEFT JOIN my_pages pages ON anc.authorid=pages.ownerid 
		WHERE  {$qArticleType} anc.topcontent IN ({$topcontent}) {$qHeadid} {$search} {$startdate} {$enddate} {$filter['keywordsearch']} {$filter['categorysearch']} {$filter['citysearch']} {$filter['postedsearch']} {$filter['fromwhosearch']} {$qStatus} {$qHirarki} {$qCategories} {$qCheckCheckin}
		{$filter['groupbyfilter']}		";
		$total = $this->apps->fetch($sql);
		...
		$sql = "
			SELECT 			anc.id,anc.title,anc.brief,anc.image,anc.thumbnail_image,anc.slider_image,DATE_FORMAT(anc.posted_date, '%d-%m-%Y')as date_posted,anc.posted_date,anc.expired_date,anc.file,anc.url,anc.fromwho,anc.tags,anc.authorid,anc.topcontent,anc.cityid ,anc.articleType,anc.can_save,anc.n_status,pages.type pagetypes,anc.categoryid
			FROM {$this->dbshema}_news_content anc
			LEFT JOIN my_pages pages ON anc.authorid=pages.ownerid 
		...
		// $this->logger->log(json_encode($this->apps->user));
		if($rqData) {
			//CEK DETAIL IMAGE FROM FOLDER
			//IF IS ARTICLE, IMAGE BANNER DO NOT SHOWN
			foreach($rqData as $key => $val){
	...
	function getCommentModeration($start=null,$limit=10,$type=null) {
		...
		if ($this->apps->_g('page')=='moderation') {
			...
			if ($this->apps->_p('publishedtype')) { 
				...
				if($this->apps->_p('publishedtype')=='3')
				{
				
					$qStatus = " AND n_status  IN (3,0)  ";
					$qPublishStatus = " AND n_status IN (3,0)  ";
				}
			}
			else
			{
	...
	function getDetailArticle($start=null,$limit=1,$contenttype=false) {		
		...
		WHERE  n_status IN (0,1,2,3)  {$qid}  {$qCategory} {$qType} ";
	// pr($sql);
		...
		//n_status IN (0,1)
		$sql = "
		SELECT acontent.*,DATE_FORMAT(acontent.posted_date, '%d-%m-%Y')as date_posted,
		DATEDIFF(NOW(),acontent.posted_date) as diff_date,acategory.point ,acategory.category,anct.type  
		FROM {$this->dbshema}_news_content acontent
	...
	function addUploadImage($data=false,$type=NULL){
		...
		$brandid = 0;
		if($type==6) $brandid = intval($this->apps->_p('brandid'));
		...
		$posted_date =="";
		$expired_date ="";
		if($this->apps->_p("posted_date"))
			{
				
				$posted_date = strip_tags(date('Y-m-d',strtotime($this->apps->_p("posted_date"))));
			}
		$posted_date_times = strip_tags($this->apps->_p('times'));
		if($this->apps->_p("expired_date"))
			{
				
				$expired_date = strip_tags(date('Y-m-d',strtotime($this->apps->_p("expired_date"))));
			}
		
		$city_event = intval($this->apps->_p('city_event'));
		$topcontent = intval($this->apps->_p('sticky'));
		$type_brand = intval($this->apps->_p('upload_type'));
		$iscms = intval($this->apps->_p('iscms'));
		...
		//user yang upload 1,2,3,4,5,6,7
		// bukan dari cms
		// pake fid
		
		if($iscms==0)$fid[] = $this->apps->_p('fid');
		else  $fid[] = $this->apps->_p('recipientid');
		$fid[] = $this->apps->_p('fidbrand');
		$fid[] = $this->apps->_p('fidarea');
		$fid[] = $this->apps->_p('fidpl');
		// $this->logger->log('friend data '.$type_brand);	
		// $this->logger->log('friend data '.$this->apps->_p('fid'));	
		// $this->logger->log('friend data '.$this->apps->_p('recipientid'));	
		// $this->logger->log('friend data '.json_encode($fid));	
		...
		if($type_brand>0){
			...
			$sql ="
			INSERT INTO {$this->dbshema}_news_content (cityid,brief,title,content,tags,image,articleType,created_date,posted_date,expired_date,authorid,fromwho,n_status,url,file,topcontent,categoryid,headid) 
			VALUES ('{$city_event}','{$brief}','{$title}','{$description}','{$tags}','{$image}',{$type},NOW(),'{$posted_date}','{$expired_date}','{$authorid}','{$fromwho}',{$this->moderation},\"{$url}\",\"{$file}\",{$topcontent},{$categoryid},'{$brandid}')
			";
		}
							...
							foreach($frienddata as $val){
								if($val['fid']!=0){
									...
									if($type!=5){
										...
										$sql =" 
											SELECT sm.id ,pages.type frole
											FROM social_member sm 
											LEFT JOIN my_pages pages ON pages.ownerid = sm.id
											WHERE sm.id IN ({$val['fid']})    LIMIT 1 
										";
									 
										$fData = $this->apps->fetch($sql);	
										if($fData){	
											if($fData['frole']==1) $n_statustags = 1;
											else	{
												 $n_statustags = 0;									
											}
										}
	...
	function getFriendTagged($strCid=null,$tagin=0,$authorid=true){
		...
		$arrEntourageFid = false;
				...
				if($arrSocialFid) {
					...
					$socialfid = $this->apps->usermodHelper->socialdata($strsocialfid);
				...
				if($arrEntourageFid) {
					...
					$entouragefid = $this->apps->usermodHelper->entouragedata($strentouragefid,false);
	...
	function getFriendTaggedByPlan($strCid=null,$tagin=0,$authorid=true){
		...
		if($authorid)$quserid = " AND userid={$this->uid} ";
		$plantypes = $this->apps->user->plantypes;
		if($plantypes=='BA')$quserid = " AND userid={$this->uid} ";
		else $quserid = "";
	...
	function addFriendTags($cid=0,$fid=0,$ftype='pass',$gallery=false,$message=false,$n_status=1,$gid=0){
		...
		if($this->apps->getLastInsertId()>0) {
			if($message){
				// $this->apps->messagemodHelper->createMessage($fid,$message,$ftype);
	...
	function unContentPost(){
		...
		if($type==4){
			$qauthorid = " AND authorid={$this->apps->user->leaderdetail->brandid} ";
		}
		
		//$this->logger->log(" authorid : ".$qauthorid);
		//$this->logger->log(" user : ".$this->apps->user->leaderdetail->type);
		...
		$sql = "UPDATE {$this->dbshema}_news_content SET n_status = {$publishedtype} WHERE id = '{$cid}' AND fromwho=1  LIMIT 1";
		//pr($sql);
	...
	function unCommentPost(){
		...
		if($this->apps->user->leaderdetail->type==100) $qauthorid = ""; 
		if($this->apps->user->leaderdetail->type==101) $qauthorid = ""; 
		$sql = "UPDATE {$this->dbshema}_news_content_comment SET n_status = {$publishedtype} WHERE id = '{$id}'  LIMIT 1";		
		if ($this->apps->query($sql)) {
			return true;
		} else return false;
		return false;
	...
	function editContentArticle($data=false){
		...
		if($this->apps->_p('posted_date'))
		{
			$qSet['posted_date'] = strip_tags(date('Y-m-d',strtotime($this->apps->_p('posted_date'))));
		}
		$times = strip_tags($this->apps->_p('times'));
		if($this->apps->_p('expired_date'))
		{
			$qSet['expired_date'] = strip_tags(date('Y-m-d',strtotime($this->apps->_p('expired_date'))));
		}
	...
	function getCheckin($cidstr=false){
		...
		foreach($qData as $key => $val){	
			...
			$qData[$key]['venue'] = html_entity_decode(ucwords(strtolower(str_replace("&amp;amp;","&amp;",$val['venue']))));
	...
	function getGalleryTypeContent($start=null,$limit=12) {
		...
		//RUN FILTER ENGINE, KEYWORDSEARCH , CONTENTSEARCH 
		$filter = $this->apps->searchmodHelper->filterEngine($limit,false,false);
	...
	function removeContentNews($id=null){
		global $CONFIG;

		$sql = "UPDATE {$this->dbshema}_news_content 
		SET n_status='3'
		WHERE id = '{$id}' and authorid='{$this->uid}' ";
		if ($this->apps->query($sql)) {
			return true;
		}
		return false;
	...
	function listBA(){
		...
		$sql="SELECT sm.id, sm.name, sm.last_name, mp.city, mp.brandid, mp.type
				...
				ON sm.id=mp.ownerid
				WHERE mp.type=1 {$filterArea} {$filterBrand} AND sm.n_status= 1 ";
		$rs = $this->apps->fetch($sql,1);

		if($rs) return $rs;
		return false;
	}
function listBA2($type='sba',$leadid=false){
		...
		$brandid = strip_tags($this->apps->_p('brand'));
		$areaid = strip_tags($this->apps->_p('cityid'));
		...
		$qareaid = "";
		if($areaid){
				$qareaid = " AND pages.city IN ({$areaid})   ";
		}
	...
	function loadCity(){
		...
		$sql="SELECT *
				...
				WHERE id IN (140,199,215,292,318,407,451,313,373,2265,255)";
		$rs = $this->apps->fetch($sql,1);
