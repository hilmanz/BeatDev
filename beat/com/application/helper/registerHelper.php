...
class registerHelper {
		...
		if($qData){
			...
			if($rqData){
				...
				$this->createrolefriends($qData['id'],$otherid,$brandid,$brandsubid,$areaid);
							...
							sleep(1);
							$sql = "
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$qData['id']},NOW(),'comment',1)";
							$this->apps->query($sql);
							sleep(1);
							$sql = "
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$qData['id']},NOW(),'tags',1)";
							$this->apps->query($sql);	
							sleep(1);							
							$sql = " 
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$qData['id']},NOW(),'emoticon',1 )";
							$this->apps->query($sql);
							sleep(1);
							$sql = " 
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$qData['id']},NOW(),'inbox',1 ) ";
							$this->apps->query($sql);
				...
				$sql = "
					INSERT INTO my_pages (ownerid ,	name, 	description ,	type 	,img ,	otherid,	brandid 	,brandsubid ,	areaid ,	city 	,created_date ,	closed_date,n_status,masterrole) 
				...
				// pr($sql);
				 $this->apps->query($sql);
					if($this->apps->getLastInsertId()>0)  {
							if($type==1||$type==2) $this->findmyfrienddefault($qData['id']);
							else $this->findtoclassfrienddefault($qData['id']);
							// $this->createrolefriends($qData['id'],$otherid,$brandid,$brandsubid,$areaid);
				...
				// pr($sql);exit;
				 $this->apps->query($sql);
					if($this->apps->getLastInsertId()>0)  {
						...
						if($type==1||$type==2) $this->findmyfrienddefault($lastID);
						else $this->findtoclassfrienddefault($lastID);
						
							sleep(1);
							$sql = "
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$lastID},NOW(),'comment',1)";
							$this->apps->query($sql);
							sleep(1);
							$sql = "
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$lastID},NOW(),'tags',1)";
							$this->apps->query($sql);	
							sleep(1);							
							$sql = " 
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$lastID},NOW(),'emoticon',1 )";
							$this->apps->query($sql);
							sleep(1);
							$sql = " 
							INSERT IGNORE INTO `my_news_letter`  (userid,`datenewsletter`,`type`,`n_status`)
							VALUES ({$lastID},NOW(),'inbox',1 ) ";
							$this->apps->query($sql); 
	...
	$sql =" 
		SELECT sm.id,sm.name FROM social_member sm
 ...
 // id IN (140,199,215,292,318,407,451,313,373,2265,255)
		$sql="SELECT *
