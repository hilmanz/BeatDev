...
class activitymodHelper {
	...
	function getA360activity($start=0,$limit=5,$user=false,$thisBand=false,$followeronly=false,$n_status=3,$push=false){
		...
		$theactivity = $this->apps->activitymodHelper->getactivitytype($articleActivity);	
		/* get article of user */
		...
		$socialactivitydata = $this->apps->activitymodHelper->getactivitytype($socialActivity);
		...
		if($activityIDarr) {
			...
			if($followeronly){
				$arrFriends = false;
				$friendid = false;
				$friendsData = $this->apps->usermodHelper->getFriends(false);
	...
	function getContentData($contentid){
			...
			if($arrContent){
				$arrContent = $this->apps->contentmodHelper->getStatistictArticle($arrContent);
			}
