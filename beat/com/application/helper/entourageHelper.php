...
class entourageHelper {
  ...
  function getBrand(){
		...
		$data['savemop'] = false;
			...
			if($this->apps->user->id!=$isexisiting){
				if($usertype==2||$usertype==0){
					...
					if($isexisiting!=0) $this->apps->query($sql);
					//$this->logger->log($sql);
	...
	function synchenturage_batch(){
		...
		$sql = "SELECT * FROM my_entourage WHERE n_status = 0 ORDER BY register_date ASC LIMIT 20 ";
		$qData = $this->apps->fetch($sql,1);
	...
	function entouragechart($streid=null,$start=0,$limit=10,$all=false,$summary=false,$alldata=false){
		...
		$sql = "	
		SELECT COUNT(*) total FROM my_entourage entou 	
		WHERE  1 {$qUserid} {$qEntourage} {$qFilter} {$qFilterAlphabet} {$qDate}  AND entou.n_status IN (0,1,2) ";	
		$total = $this->apps->fetch($sql);		
		//pr($total);
		  // pr($total);
		if(!$total)return  array();
		...
		WHERE 1 {$qUserid} {$qEntourage} {$qFilter} {$qFilterAlphabet} {$qDate}   AND entou.n_status IN (0,1,2)  {$qLimit}   ";		
		
		$qData = $this->apps->fetch($sql,1);
// pr($sql);
$this->logger->log(" kluar sini : ".$sql);
		if($qData) {
			$brandid = @$this->apps->user->branddetail;
			if($brandid){
				foreach($brandid as $val){
