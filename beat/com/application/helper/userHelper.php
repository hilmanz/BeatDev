...
class userHelper {
	...
	function getUserProfile(){
		...
		if($uid!=0 || $uid!=null) {
			...
			$plan = $this->apps->contentHelper->getArticleContent(null,30,4,array(0,3),"plan",false,false,false,true,true,true,false,true);
			if($plan['result']){
			...
			$baengagement = $this->getbaengagement(10);
			
			$notification = $this->apps->activityHelper->getA360activity(0,10,false,false,false,'3',false);	
			...
			$challenge = $this->apps->contentHelper->getArticleContent(null,50,3,array(0,3),"challenge" );
			...
			if($challenge['result']){
				foreach($challenge['result'] as $key =>$val){
					...
					$challengeData = $this->apps->contentHelper->getChallangeData($val['id']);
					if($challengeData){
 ...
 */					$result = $this->apps->query($sql);	
	...
	function checkuserpassword(){
		...
		$sql = "SELECT * FROM social_member WHERE id={$this->apps->user->id} AND login_count=0 LIMIT 1";
		$rs = $this->apps->fetch($sql);
		// pr($rs);die;
		if($rs) return false;
		return true;
