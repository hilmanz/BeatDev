...
class notification  extends ServiceAPI{
	...
	function read(){
		$data = $this->activityHelper->readnotifuser();	
		return $data;
	}
	function currentsetting(){
		$data = $this->apnsHelper->getcurrentsetting();	
		return $data;
	}
	
	function inbox(){
		$data = $this->activityHelper->getnotificationuser(10,0,'inbox');	
		if(!$data['content'])  $data['content'] = array();	
		return $data;
	}
